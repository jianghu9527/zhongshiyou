...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.cleardataButton.setOnClickListener {
             LogMangeUtil.d("-------------------------","----------cleardataButton------------创建缓存文件---");
...
//            MediaStoreUtils(). saveImageToPublicDirectory(this, bitmap, "example_image")
    ...
    fun  creatFileDir() {
        ...
        LogMangeUtil.d("------------------初始化下载路径------------","------------------Environment：${Environment.getExternalStorageState() }");


        LogMangeUtil.d("------------------初始化下载路径------------","------------------public_img：${public_img}");
        LogMangeUtil.d("------------------初始化下载路径------------","------------------public_video：${public_video}");
...
//        LogMangeUtil.d("","---------------mcreateOrExistsDir------created:${mcreateOrExistsDir}");
      ...
      var  file = File(public_video)
        if (!file.exists()) {
            file.mkdirs()
            LogMangeUtil.d("","---------------newFile----------file-----------能在data的data下创建-----created:"+file.absolutePath);
    ...
    var mfiles=    Environment.getExternalStorageDirectory();
        ...
        if (!mnfile.exists()) {
          var mesfile=  mnfile.mkdirs()
            ...
            try {
                ...
                LogMangeUtil.d("","------------mnfile-------mesfile--11------messfile:"+messfile);
            }catch (e:Exception){
                LogMangeUtil.d("","------------mnfile-------mesfile--message------messfile:"+e.message)
            }
        ...
        if (!appSpecificExternalDir.exists()) {
            ...
            LogMangeUtil.d("-----------------------------MainActivity", "----------------appSpecificExternalDir:${appSpecificExternalDir}")
            if (isCreated) {
                LogMangeUtil.d("-----------------------------MainActivity", "------------------------Directory created")
            } else {
                LogMangeUtil.d("------------------------MainActivity", "------------------------Directory not created")
            }
        }
